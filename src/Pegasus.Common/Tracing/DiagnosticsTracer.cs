...
namespace Pegasus.Common.Tracing
{
    ...
    public class DiagnosticsTracer : ITracer
    {
        ...
        {
            Debug.WriteLine($"Begin '{ruleName}' at ({cursor.Line},{cursor.Column}) with state key {cursor.StateKey}");
        }
        ...
        public void TraceRuleExit<T>(string ruleName, Cursor cursor, IParseResult<T> parseResult)
        {
            ...
            Debug.WriteLine($"End '{ruleName}' with {(success ? "success" : "failure")} at ({cursor.Line},{cursor.Column}) with state key {cursor.StateKey}");
        }
    }
}
